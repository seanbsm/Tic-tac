...
double calculate_3N_potential_energy(double* state_3N_bra_array,
    ...
    const std::string model = "Idaho_N3LO";
                    ...
                    if (S==Sp and J==Jp and T==Tp and abs(L-Lp)<3){
                        ...
                        for (int T2z=-T; T2z<=T; T2z++){
                            ...
                            if (T2z==-1){       // nn-pair, lone p-particle
                                pot_ptr_nn->V(pp, p, coupled, S, J, T, V_array);
                                CG_coeff = CGcoeff(T3, T3z, T, T2z, 1./2, +1./2);
                            }
                            else if (T2z==0){   // np-pair, lone n-particle
                                pot_ptr_np->V(pp, p, coupled, S, J, T, V_array);
                                CG_coeff = CGcoeff(T3, T3z, T, T2z, 1./2, -1./2);
                            }
                            else{ /* We ignore pp-interactions for now */
                                continue;
                            }
                            // ss 00 mm pm mp pp
    ...
    return hbarc*hbarc*potential_energy_unormalised / inner_product;
}