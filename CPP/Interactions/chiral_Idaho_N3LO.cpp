...
chiral_Idaho_N3LO::chiral_Idaho_N3LO(){
	...
	chp_set_Idaho_N3LO(&parameters[0]);
}

void chiral_Idaho_N3LO::setup_label_vector_combinations(){
...
void chiral_Idaho_N3LO::setSystem(int Tz_in){
	Tz = -Tz_in;
}

void chiral_Idaho_N3LO::update_parameters(){
	chp_set_Idaho_N3LO(&parameters[0]);
}

void chiral_Idaho_N3LO::setParameter(int index, floatType value){
	parameters[index] = value;
}

void chiral_Idaho_N3LO::retrieveSymmetric(bool &symmetric_in){
	symmetric_in = symmetric;
}

void chiral_Idaho_N3LO::retrieve_state_dep_labels(std::vector<std::string> &state_dep_labels_in){
	state_dep_labels_in = state_dep_labels;
}

void chiral_Idaho_N3LO::retrieve_state_dep_indices(std::vector<int> &state_dep_idx_lookup_in){
	state_dep_idx_lookup_in = state_dep_idx_lookup;
}

void chiral_Idaho_N3LO::retrieve_state_indep_labels(std::vector<std::string> &state_indep_labels_in){
	state_indep_labels_in = state_indep_labels;
}

void chiral_Idaho_N3LO::retrieve_state_indep_indices(std::vector<int> &state_indep_idx_lookup_in){
	state_indep_idx_lookup_in = state_indep_idx_lookup;
}

void chiral_Idaho_N3LO::retrieve_state_list(std::vector<int> &state_list_in){
	state_list_in = state_list;
}

void chiral_Idaho_N3LO::V(double &qi, double &qo, bool coupled, int &S, int &J, int &T, double *Varray){
